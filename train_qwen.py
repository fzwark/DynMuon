...
class Hyperparameters:
    ...
    wandb_project_name: str = "dynmuon-test"

    # Optimizer
    optimizer: str = "dynmuon"
    ...
    dynmuon_pmax: float = 1.0
    dynmuon_pmin: float = -0.01
    dynmuon_w: float = 0.04
    dynmuon_tau: float = 0.04
...
def parse_cli_args():
    ...
    parser.add_argument("--dynmuon_pmax", type=float, default=None)
    parser.add_argument("--dynmuon_pmin", type=float, default=None)
    parser.add_argument(
        "--dynmuon_w",
        type=float,
        default=None,
        help="DynMuon logistic scheduler width ratio",
    )
    parser.add_argument(
        "--dynmuon_tau",
        type=float,
        default=None,
        help="DynMuon logistic scheduler tau ratio",
    )
...
def init_optimizer(
    ...
    def _split_matrix_scalar(params):
    ...
    # transformer blocks: split into matrix vs scalar
    block_matrix_params, block_scalar_params = _split_matrix_scalar(
        model.transformer.h.parameters()
    )

    # final norm should go to scalar params, not Muon
    final_norm_params = []
    if hasattr(model.transformer, "norm"):
        final_norm_params = _dedup_params(model.transformer.norm.parameters())

    # embedding / lm_head keep separate
    embedding_params = _dedup_params(model.transformer.wte.parameters())

    embedding_param_ids = {id(p) for p in embedding_params}
    lm_head_params = [
        p for p in _dedup_params(model.lm_head.parameters())
        if id(p) not in embedding_param_ids
    ]

    # Muon/DynMuon main group: matrices only
    matrix_params = block_matrix_params

    # scalar params: block norms + final norm
    scalar_params = _dedup_params(block_scalar_params + final_norm_params)
    ...
    if len(scalar_params) > 0:
        param_groups.append(
            dict(
                ...
                lr= hp.scalar_lr, # 0.002, # hp.scalar_lr,
                betas=(0.9, 0.95),
                weight_decay=0,
            )
        )
    ...
    if len(embedding_params) > 0:
        param_groups.append(
            dict(
                ...
                algorithm=hp.scalar_opt,
                lr= hp.scalar_lr, #  0.005, # hp.scalar_lr,
                betas=(0.90, 0.95),
                weight_decay=0,
            )
        )
    ...
    if hp.optimizer == "dion":
        ...
        if device_mesh is not None:
            ...
            distributed_mesh = ddp_model.process_group  # using ProcessGroup for DDP
            comm_method = "all-gather"
        print0(f"DynMuon LR adjust method: {hp.adjust_lr}")
        print0(f"Triton Newton-Schulz kernels: {not cli_args.no_triton}")
        print0(f"Distributed DynMuon using: {comm_method}")
        opt = DynMuon(
            param_groups,
            distributed_mesh=distributed_mesh,
            lr=hp.lr,
            ...
            adjust_lr=hp.adjust_lr,
            use_triton=False,
            total_steps=hp.num_iterations,
            p_max=hp.dynmuon_pmax,
            p_min=hp.dynmuon_pmin,
            width_ratio=hp.dynmuon_w,
            tau_ratio=hp.dynmuon_tau,
        )
...
def main():
    ...
    run_name = f"{hp.optimizer}"
    # if "dion" in hp.optimizer or "dion2" in hp.optimizer or "sgs" in hp.optimizer or "svd" in hp.optimizer:
    ...
    # run_name += f"_{hp.n_layer}_{hp.model_dim}_{hp.n_head}_{hp.lr}"
    run_name += (
        f"_qwen_{hp.n_layer}_{hp.model_dim}_{hp.n_head}"
        f"_{hp.n_kv_head}_{hp.intermediate_size}_{hp.lr}"
    )
    ...
    for step in range(start_step, hp.num_iterations + 1):
        ...
        total_train_loss = torch.zeros((), device=model.device if hasattr(model, 'device') else "cuda")
        ...
        train_loss = total_train_loss / grad_accum_steps
        # Gradient norm
