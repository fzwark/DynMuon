...
class DynMuon(Optimizer):
    ...
    def __init__(
        ...
        if newton_schulz_func is not None:
            ...
            self._newton_schulz_func = dynmuon_spectral_transform
        else:
            self._newton_schulz_func = dynmuon_spectral_transform 

        self._total_steps = int(total_steps)

        self.log_psf = Logistic_Scheduler(
            p_max=p_max,
            p_min=p_min,
            tau_ratio=tau_ratio,
...
def fast_spectral(
    ...
    Y_mu = newton_schulz_triton(Xn, epsilon=epsilon).to(torch.float32)
